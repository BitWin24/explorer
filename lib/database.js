        ...
        if (nodeBlock) {
          Tx.find({blockindex: {$eq: blockIndex}}).sort({timestamp: 'desc'}).exec(function(err, txsToCheck) {
            ...
            if (badTxs) {
              ...
              console.log('Rolled back to stable block');
...
module.exports = {
  ...
  get_mnpeers: function(cb) {
    Mnpeers.find({}, function(err, mnpeers) {
      if (err) {
        ...
        let filtered = mnpeers.filter(peer => peer.status !== "EXPIRED" && peer.status !== "REMOVE");
        filtered.sort((a, b) => {
  ...
  delete_mnpeers: function(cb) {
    Mnpeers.remove({}, function(err) {
      if (err) {
        console.log("Failed to delete from mnpeers");
      }
      return cb();
    });
  }
