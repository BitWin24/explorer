      ...
      lib.get_blockhash(start + x, function(blockhash){
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            ...
            if (block) {
              Tx.find({}).where('blockindex').gt(end-1).sort({timestamp: 'desc'}).limit(settings.index.last_txs).exec(function(err, txs){
                if (txs[0]){
                console.log("My:             " + txs[0]["blockhash"], txs[0]["blockindex"]);
                }
                console.log("Their previous: " + block["previousblockhash"])
