var mongoose = require('mongoose')
  ...
  , Mnpeers = require('../models/mnpeers')
...
function find_addresses(hashes, cb) {
  Address.find({
      a_id: {
        $in: hashes
      }
...
function find_txs(txid_list, cb) {
  Tx.find({txid: { $in: txid_list }}, function(err, txs) {
    if(txs) {
      txs.sort(function(a, b) {
        return txid_list.indexOf(a.txid) - txid_list.indexOf(b.txid);
      });
      return cb(txs);
    } else {
      return cb(null);
...
function update_address_rollback(hash, txid, amount, type, cb) {
  // Check if address exists
  find_address(hash, function(address) {
      if (address) {
        // if coinbase (new coins PoW), update sent only and return cb.
        if ( hash == 'coinbase' ) {
          ...
          lib.is_unique(address.txs, txid, function(unique, index) {
            ...
            if (type == 'vin') {
              ...
              let i = 0;
              while (tx_index[i]) {
                if (tx_index[i]["address"] == txid) {
                  tx_index = i;
                }
              }
              tx_array.splice(tx_index, 1);
              tx_array.push({addresses: txid, type: type});
              Address.update({a_id:hash}, {
                 $pull: {
                  txs: [tx_index] 
        ...
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                var i = loop.iteration();
                update_address_rollback(nvin[i].addresses, txid, nvin[i].amount, 'vin', function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  var t = subloop.iteration();
                  if (vout[t].addresses) {
                    update_address_rollback(vout[t].addresses, txid, vout[t].amount, 'vout', function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
          ...
          return 0;//cb('block not found: ' + tx.blockhash);
        }
      });
    } else {
      return 0;//cb('tx not found: ' + txid);
    }
  });
}

function rollback(explorerBlockIndex, blockIndex, cb) {                  
  lib.get_blockhash(blockIndex, function(blockhash) {
    if (blockhash) {
      lib.get_block(blockhash, function(nodeBlock) {
        if (nodeBlock) {
          Tx.find({blockindex: {$eq: blockIndex}}).sort({timestamp: 'desc'}).exec(function(err, txsToCheck) {
            let badTxs = false;
            txsToCheck.forEach(txToCheck => {
              if (txToCheck["blockhash"] != nodeBlock["hash"]) {
                badTxs = true;
                Tx.find({"txid": txToCheck["txid"] }).remove().exec();
              }
              if (badTxs) {
                rollback_tx(txToCheck["txid"]);
                rollback(explorerBlockIndex - 1, blockIndex - 1, cb);

              } else {
                return cb(explorerBlockIndex); // stable block
              }
            });
          });
        }
...
module.exports = {
  ...
  get_addresses: function(hashes, cb) {
    find_addresses(hashes, function(addresses){
      return cb(addresses);
    });
  },
  ...
  get_txs: function(txid_list, cb) {
    find_txs(txid_list, function(txs){
      return cb(txs);
    });
  },

  get_block_txs: function(block, cb) {
    find_txs(block.tx, function(txs){
      return cb(txs);
    });
  },
  ...
  handle_rollback: function(start, cb){
    lib.get_blockhash(start, function(blockhash){
      if (blockhash) {
        lib.get_block(blockhash, function(block) {
          if (block) {
            Tx.find({blockindex: {$gt: block["height"] - 1}}).sort({timestamp: 'desc'}).limit(settings.index.last_txs).exec(function(err, txs){    
              if (txs && txs[0]){
                console.log("My top:         " + txs[0]["blockhash"], txs[0]["blockindex"]);
              }
              // case 1: Update complete, current explorer block hash != current node block hash
              if (txs && txs[0] && (txs[0]["blockindex"] == block["height"]) && (txs[0]["blockhash"] != block["hash"])) {
                console.log("FORK (type1)");
                rollback(txs[0]["blockindex"], block["height"], cb); // stable block
              }
              // case 2: Update in process, top_explorer_block hash != node block index(top_explorer_block + 1) => previous hash
              else if (txs && txs[0] && (txs[0]["blockindex"] == block["height"]) && (txs[0]["blockhash"] != block["hash"])) {
                console.log("FORK (type2)");
                rollback(txs[0]["blockindex"], block["height"], cb); // stable block
              }
              else {
                console.log("Hashes are equivalent");
                return cb(start); // fork not detected
              }
            });
          }
        });
  ...
  create_mnpeer: function(params, cb) {
    var newMnpeer = new Mnpeers(params);
    newMnpeer.save(function(err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
  ...
  find_mnpeer: function(address, cb) {
    Mnpeers.findOne({address: address}, function(err, mnpeer) {
      if (err) {
        return cb(null);
      } else {
        if (mnpeer) {
  ...
  get_mnpeers: function(cb) {
    Mnpeers.find({}, function(err, mnpeers) {
      if (err) {
        return cb([]);
      } else {
        let mnpeersNodDouble = [];
        let index = {};
        mnpeers.map(v => {
          if (!(v["name"] in index)) {
            index[v["ip"]] = v
          } else if (index[v["ip"]]["lastSeen"] < v["lastSeen"]) {
            index[v["ip"]] = v
          }
        });
        for (let k in index) {
          if (index[k].status !== "EXPIRED" && index[k].status !== "REMOVE") {
            mnpeersNodDouble.push(index[k])
          }
        }
        mnpeersNodDouble.sort((a, b) => {
          return a.rank - b.rank;
        });
        return cb(mnpeersNodDouble);
      }
    });
  }
};
