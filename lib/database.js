...
function rollback(explorerBlockIndex, blockIndex, cb) {                  
  console.log("EXPLR, NODE block indexes", explorerBlockIndex, blockIndex);
        ...
        if (nodeBlock) {
          Tx.find({blockindex: {$eq: blockIndex}}).sort({timestamp: 'desc'}).exec(function(err, txsToCheck) {
            ...
            txsToCheck.forEach(txToCheck => {
              // console.log("Txblockhash / blockhash", txToCheck["blockhash"], nodeBlock["hash"]);
              if (txToCheck["blockhash"] != nodeBlock["hash"]) {
                badTxs = true;
                console.log("Bad tx id to delete: ", txToCheck["txid"]);
                Tx.find({"txid": txToCheck["txid"] }).remove().exec();
              }
              if (badTxs) {
                rollback_tx(txToCheck, function () {});
                rollback(explorerBlockIndex - 1, blockIndex - 1, cb);
              } else {
                console.log("+_=-=_+-=- ", explorerBlockIndex)
...
module.exports = {
        ...
        lib.get_block(blockhash, function(block) {
          ...
          if (block) {
            Tx.find({blockindex: {$gt: block["height"] - 1}}).sort({timestamp: 'desc'}).limit(settings.index.last_txs).exec(function(err, txs){
              ...
              // console.log("===> \n"); 
              // console.log(block);
              console.log("Their previous: " + block["previousblockhash"], block["height"] - 1)
              console.log("Their:          " + block["hash"], block["height"]);
  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    console.log("update_tx_db", {coin, start, end, timeout, cb});
    var complete = false;
