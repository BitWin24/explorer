...
function rollback(explorerBlockIndex, blockIndex) {                  
          ...
          Tx.find({blockindex: {$eq: blockIndex}}).sort({timestamp: 'desc'}).exec(function(err, txsToCheck) {
            ...
            txsToCheck.forEach(txToCheck => {
              ...
              if (txToCheck["blockhash"] != nodeBlock["hash"]) {
                badTxs = true;
                console.log("Bad tx id: ", txToCheck["txid"]);
                Tx.find({"txid": txToCheck["txid"] }).remove().exec();
              }
              if (badTxs) {
                rollback(explorerBlockIndex - 1, blockIndex - 1);
              }
            });
          });
        }
...
module.exports = {
  ...
  // let isBadTx = false;
  // do {
  //   console.log("EXPLR, NODE block indexes", txs[0]["blockindex"], blockIndex);
  //   lib.get_blockhash(blockIndex, function(blockhash){
  //     if (blockhash) {
  //       lib.get_block(blockhash, function(nodeBlock) {
  //         if (nodeBlock) {
  ...
  //             });
  //             if (isBadTx) {
  //               console.log("REALY BAD BLOCK!!!!!!!!!!!! cancel all txs");
  //               blockIndex -= 1;
  //             } else {
  //               isBadTx = false;
  //             }
  ...
  //     }
  //   });
  //   console.log("---------------------> continiue cycle? ", isBadTx);
  // } while (isBadTx);


  // resolve txs in db after fork
  // transaction is good if block hash == node block hash && other transactions with the same index are good
 
  // updates tx, address & richlist db's; called by sync.js
  update_tx_db: function(coin, start, end, timeout, cb) {
    console.log({coin, start, end, timeout, cb});
    var complete = false;
            ...
            if (block) {
              ...
              Tx.find({blockindex: {$gt: block["height"] - 1}}).sort({timestamp: 'desc'}).limit(settings.index.last_txs).exec(function(err, txs){
                ...
                if (txs && txs[0] && (txs[0]["blockindex"] == block["height"]) && (txs[0]["blockhash"] != block["hash"])) {
                  ...
                  rollback(txs[0]["blockindex"], block["height"]);
                }
